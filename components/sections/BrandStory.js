      ...
      <div className="section-container">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative">
            ...
            <div className="absolute bottom-8 left-6 right-6 glass-dark p-4 border-l-2 border-gold">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.2 }}>
            ...
            <div className="space-y-5 text-white/60 font-sans text-sm leading-relaxed">
            ...
            <div className="grid grid-cols-3 gap-6 my-10 py-8 border-y border-white/10">
              {[{ n: '500+', l: 'Designs' }, { n: '10K+', l: 'Customers' }, { n: '6+', l: 'Collections' }].map(s => (
                <div key={s.l} className="text-center">
                  <p className="font-serif text-3xl text-gold mb-1">{s.n}</p>
                  <p className="text-white/35 text-xs font-sans tracking-wide">{s.l}</p>
                </div>
              ))}
            </div>
